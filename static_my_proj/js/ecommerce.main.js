...
var stripeFormModule = $(".stripe-payment-form")
	...
	var stripeModuleBtnTitle = stripeFormModule.attr("data-btn-title") || "Adicionar cartão"
	...
	if (paymentForm.length > 1) {
		alert("Apenas possível um form de pagamento por página")
		...
		form.on('submit', function(event) {
			...
			var loadTime = 1500
			var currentTimeout;
			var errorHtml = "<i class='fa fa-warning'></i> Ocorreu um erro"
			var errorClasses = "btn btn-danger disabled my-3"
			var loadingHtml = "<i class='fa fa-spin fa-spinner'></i> Carregando..."
		...
		function stripeTokenHandler(nextUrl, token) {
			...
			$.ajax({
				...
				success: function(data){
					var successMsg = data.message || "Concluido! O seu cartão foi adicionado."
					card.clear()
					if (nextUrl){
						successMsg = successMsg + "<br/><br/><i class='fa fa-spin fa-spinner'></i> Redirecionando..."
					}
					if ($.alert){
						$.alert(successMsg)
					} else {
				...
				error: function(error){
					console.log(error)
					$.alert({title: "Ocorreu um erro", content: "Por favor tente adicionar o seu cartão novamente."})
				}
			})
		}
	}
