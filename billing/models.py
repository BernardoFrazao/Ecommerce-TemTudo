...
class BillingProfileManager(models.Manager):
	def new_or_get(self, request):
		...
		if user.is_authenticated:
			#checkout de um utilizador com o login feito;lembra-se dos dados de pagamento
			obj, created = self.model.objects.get_or_create(user=user, email=user.email)
		elif guest_email_id is not None:
			#checkout de um utilizador convidado; autoreload das informações de pagamento
...
class ChargeManager(models.Manager):
	def do(self, billing_profile, order_obj, card=None):
		...
		if card_obj is None:
			return False, "Sem cartões disponíveis"

		c = stripe.Charge.create(
				amount = int(order_obj.total * 100), # 39,99 fica 3999
				currency = "eur",
