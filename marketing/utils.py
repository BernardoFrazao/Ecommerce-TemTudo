...
def check_email(email):
    if not re.match(r".+@.+\..+", email):
        raise ValueError('O texto não é um email válido')
    return email
...
class Mailchimp(object):
	...
	def check_valid_status(self, status):
		...
		if status not in choices:
			raise ValueError("Estado de email inválido.")
		return status
	...
	def archived(self, email):
		return self.change_subscription_status(email, status='archived')
