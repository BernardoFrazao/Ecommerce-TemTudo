...
class ObjectViewed(models.Model):
	user = models.ForeignKey(User, blank=True, null=True, on_delete=models.CASCADE) # instância do utilizador
	ip_address = models.CharField(max_length=220, blank=True, null=True)
	content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE) # Tipo de conteudo
	object_id = models.PositiveIntegerField() # id do conteudo
	content_object = GenericForeignKey('content_type', 'object_id') # instância do produto
	timestamp = models.DateTimeField(auto_now_add=True)

	objects = ObjectViewedManager()

	def __str__(self):
		return "%s visto a %s" %(self.content_object, self.timestamp)

	class Meta:
		ordering = ['-timestamp'] #os guardados mais recentes aparecem primeiro
